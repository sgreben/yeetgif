...
func AdjustTint(img image.Image, weight float64, tint color.RGBA) *image.NRGBA {
	...
	parallel(0, src.h, func(ys <-chan int) {
		for y := range ys {
			...
			for x := 0; x < src.w; x++ {
				r, g, b := dst.Pix[i+0], dst.Pix[i+1], dst.Pix[i+2]
				ph, ps, pl, _ := HSLA(color.RGBA{R: r, G: g, B: b})
				hueDistDirect := math.Abs(ph - h)
				hueDistWrap := math.Abs(ph - 1 - h)
				if hueDistDirect < hueDistWrap {
					ph = ph*(1-weight) + h*weight
				} else {
					ph = ph*(1-weight) + (1-h)*weight
				}
				if ph > 1 {
					ph--
				}
				...
				ps = ps*(1-weight) + s*weight
				pl = pl*(1-weight) + l*weight
				c := RGBA(ph, ps, pl, 0)
				dst.Pix[i+0] = c.R
				dst.Pix[i+1] = c.G
				dst.Pix[i+2] = c.B
...
func AdjustHue(img image.Image, weight float64, hue float64) *image.NRGBA {
	src := newScanner(img)
	dst := image.NewNRGBA(image.Rect(0, 0, src.w, src.h))
	parallel(0, src.h, func(ys <-chan int) {
		for y := range ys {
			...
			for x := 0; x < src.w; x++ {
				r, g, b := dst.Pix[i+0], dst.Pix[i+1], dst.Pix[i+2]
				ph, ps, pl, _ := HSLA(color.RGBA{R: r, G: g, B: b})
				hueDistDirect := math.Abs(ph - hue)
				hueDistWrap := math.Abs(ph - 1 - hue)
				if hueDistDirect < hueDistWrap {
					ph = ph*(1-weight) + hue*weight
				} else {
					ph = (ph*(1-weight) + (1-hue)*weight)
				}
				c := RGBA(ph, ps, pl, 0)
				dst.Pix[i+0] = c.R
				dst.Pix[i+1] = c.G
