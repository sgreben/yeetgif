...
func FriedDistortion1(img image.Image, position image.Point, a, b, c float64) *image.NRGBA {
	...
	restX := dstW - position.X
	restY := dstH - position.Y
	radius := math.Max(
		math.Sqrt(float64(position.X*position.X+position.Y*position.Y)),
		math.Sqrt(float64(restX*restX+restY*restY)),
	)

	bgColorNRGBA := color.NRGBAModel.Convert(color.Transparent).(color.NRGBA)

	parallel(0, dstH, func(ys <-chan int) {
		for dstY := range ys {
			for dstX := 0; dstX < dstW; dstX++ {
				dx := float64(dstX) - float64(position.X)
				dy := float64(dstY) - float64(position.Y)
				dist := math.Sqrt(dx*dx + dy*dy)
				displacement := a * math.Sin(2*b*math.Pi*(dist/radius))
				weight := 1 - ((radius / dist) * c)
				displacement = weight * displacement
				if math.IsNaN(displacement) {
					displacement = 0.0
				}
				displacement = math.Min(displacement, dist)
				xf := float64(dstX) + dx*displacement
				yf := float64(dstY) + dy*displacement
				interpolatePoint(dst, dstX, dstY, src, xf, yf, bgColorNRGBA)
			}
		}
	})
